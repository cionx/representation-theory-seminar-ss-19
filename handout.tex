...
A \defemph{semistandard Young tableaux} of shape~$\lambda$ assigns to each box of the Young diagram a natural number~$1, 2, 3, \dotsc$, weakly increasing in each row and strictly increasing in each column.
...
This is the \defemph{Schur module} associated to~$\lambda$.
...
For any finite-dimensional polynomial representation~$\rho \colon \GL(V) \to \GL(W)$ the \defemph{weight space}~$W_\beta$ for the \defemph{weight}~$\beta = (\beta_1, \dotsc, \beta_m) \in \Natural^m$ is given by
\[
  W_\beta
  =
  \{
